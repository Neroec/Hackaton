import { nodes2, edges2 } from "../tmp/data";
import { dataFromIgor } from "../tmp/igor";

export function createRoutes(n, maxDistance = 0) {
  return dataFromIgor;
      ...
      for (let j = 0; j < components[i][1].length; j++) {
        ...
        for (let k = 3; k < 5; k++) {
          ...
          for (let c = 0; c < list[id2][1].length; c++) {
            ...
            if (
              edges2[id3][2] > max &&
              edges2[id3][1] + components[i][0] <= maxDistance
            ) {
              max = edges2[id3][2];
              maxId = id3;
            }
